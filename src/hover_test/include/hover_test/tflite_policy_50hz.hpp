#ifndef TFLITE_POLICY_50HZ_HPP_
#define TFLITE_POLICY_50HZ_HPP_
...
// Constants for 50Hz control (buffer size = 50)
constexpr int BUFFER_SIZE_50HZ = 50;      // 动作-状态缓冲区大小（50Hz版本）
constexpr int ACTION_DIM = 4;              // 动作维度（四旋翼控制指令）
constexpr int OBS_DIM = 9;                 // 观测维度（机体系速度3 + 机体系重力3 + 机体系到目标点距离3）
constexpr int INPUT_DIM_50HZ = BUFFER_SIZE_50HZ * (ACTION_DIM + OBS_DIM);  // 总输入维度 = 50*(4+9) = 650

/**
 * 动作-状态缓冲区类（50Hz版本）
 * 维护最近BUFFER_SIZE_50HZ个时间步的观测和动作
 */
class ActionObsBuffer50Hz {
public:
    ActionObsBuffer50Hz(int buffer_size, int obs_dim, int action_dim)
...
/**
 * TFLite策略推理器类（50Hz版本，缓冲区大小为50）
 */
class TFLitePolicyInference50Hz {
public:
    TFLitePolicyInference50Hz(const std::string& model_path)
        : buffer_(BUFFER_SIZE_50HZ, OBS_DIM, ACTION_DIM), 
        ...
        if (!model_) {
            std::cerr << "[TFLite 50Hz] Failed to load model from: " << model_path << std::endl;
            return;
        }
        ...
        if (!interpreter_) {
            std::cerr << "[TFLite 50Hz] Failed to create interpreter" << std::endl;
            return;
        }
        
        // 分配张量
        if (interpreter_->AllocateTensors() != kTfLiteOk) {
            std::cerr << "[TFLite 50Hz] Failed to allocate tensors" << std::endl;
            return;
        }
        
        initialized_ = true;
        std::cout << "[TFLite 50Hz] Model loaded successfully from: " << model_path << std::endl;
        std::cout << "[TFLite 50Hz] Buffer size: " << BUFFER_SIZE_50HZ << ", Input dim: " << INPUT_DIM_50HZ << std::endl;
        print_model_info();
    }
    ...
    void reset(const std::vector<float>& initial_obs, 
        ...
        for (int i = 0; i < BUFFER_SIZE_50HZ; ++i) {
    ...
    /**
     * 获取当前展平的缓冲区数据（用于调试）
     * @return 展平的缓冲区数据 [action(4), obs(9)] * BUFFER_SIZE_50HZ
     */
    std::vector<float> get_flattened_buffer() const {
...
private:
    ...
    std::vector<float> compute_action() {
        ...
        if (interpreter_->Invoke() != kTfLiteOk) {
            std::cerr << "[TFLite 50Hz] Failed to invoke interpreter" << std::endl;
            return std::vector<float>(ACTION_DIM, 0.0f);
        }
    ...
    void print_model_info() {
        std::cout << "[TFLite 50Hz] Model Information:" << std::endl;
...
private:
    ...
    ActionObsBuffer50Hz buffer_;
    std::vector<float> last_action_;
    bool initialized_;
    bool is_new_inference_;  // 标记上次get_action是否进行了新推理
};

#endif  // TFLITE_POLICY_50HZ_HPP_
