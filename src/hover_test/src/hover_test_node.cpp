...
HoverTestNode::HoverTestNode(ros::NodeHandle& nh, ros::NodeHandle& nh_private, int drone_id)
  : nh_(nh)
  ...
  , step_counter_(0)           // Initialize step counter
{
  ...
  nh_private_.param("action_update_period", action_update_period_, 0.1);  // 10 Hz for NN inference
  ...
  nh_private_.param("target_z", target_z_, 2.0);  // ENU: 2.0m altitude (from config)
  nh_private_.param("safety_threshold_horizontal", safety_threshold_horizontal_, 5.0);
  nh_private_.param("safety_threshold_vertical", safety_threshold_vertical_, 3.0);
  
  ROS_INFO("=== Hover Test Node for Drone %d (Neural Network Control - REQUIRED) ===", drone_id_);
  ROS_INFO("Parameters:");
  ...
  ROS_INFO("  - Safety threshold horizontal: %.2f m", safety_threshold_horizontal_);
  ROS_INFO("  - Safety threshold vertical: %.2f m", safety_threshold_vertical_);
  ...
  action_update_timer_ = nh_.createTimer(
      ...
      &HoverTestNode::action_update_callback, this);
  ...
  control_send_timer_ = nh_.createTimer(
      ...
      &HoverTestNode::control_send_callback, this);
}

void HoverTestNode::state_callback(const std_msgs::Int32::ConstPtr& msg)
{
...
void HoverTestNode::local_pos_callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
  // Position (ENU frame)
...
void HoverTestNode::local_vel_callback(const geometry_msgs::TwistStamped::ConstPtr& msg)
{
...
void HoverTestNode::action_update_callback(const ros::TimerEvent& event)
{
  if (!pose_ready_) {
  ...
  if (current_state_ == FsmState::TRAJ && hover_started_ && !hover_completed_) {
    ...
    if (!check_safety_threshold()) {
      ROS_ERROR("❌ SAFETY THRESHOLD EXCEEDED - Terminating TRAJ state and shutting down!");
    ...
    if (!neural_control_ready_ && elapsed >= mode_stabilization_delay_) {
      ...
      ROS_INFO("[NN INFERENCE TIME] First inference: %.3f ms (%.1f us)", 
...
void HoverTestNode::control_send_callback(const ros::TimerEvent& event)
{
  if (!pose_ready_) {
    return;
  }
...
void HoverTestNode::update_neural_action()
{
  ...
  // 1.5. Inference time
  ROS_INFO("[NN INFERENCE TIME] Step %d: %.3f ms (%.1f us)", 
...
bool HoverTestNode::check_safety_threshold()
{
  ...
  if (horizontal_distance > safety_threshold_horizontal_) {
    ROS_ERROR("❌ Horizontal distance to target (%.2f m) exceeds safety threshold (%.2f m)!",
              horizontal_distance, safety_threshold_horizontal_);
    ROS_ERROR("   Current position (ENU): [%.2f, %.2f, %.2f], Target (ENU): [%.2f, %.2f, %.2f]",
              current_x_, current_y_, current_z_, target_x_, target_y_, target_z_);
    return false;
  }
  
  if (vertical_distance > safety_threshold_vertical_) {
    ROS_ERROR("❌ Vertical distance to target (%.2f m) exceeds safety threshold (%.2f m)!",
